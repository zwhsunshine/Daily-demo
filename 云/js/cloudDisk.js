...
class CloudDisk{
	...
	init(){
		...
		//渲染完DOM树之后，获取元素（DOM树中的元素，在渲染之前是获取不到的），此时获取元素，之后在渲染DOM也没关系可以动态获取元素
		...
		document.addEventListener('keyup',function(ev){
			...
			if( ev.keyCode === 13 ){
				...
				//把数据中的checked的值改为false
				that.sarr[0].checked = false
		...
		//当this.arr为真的时候，说明有子级数据，循环数据，渲染页面
			...
			this.arr.forEach((e,k)=>{
				...
				div.addEventListener('click',function(ev){
					...
					//当选中之后，并且点击文件夹名字时，可以重命名
					if(e.checked && ev.target.nodeName === 'SPAN'){
						that.rename();
					}
					//否则只进行单击事件
					else{
	...
	down(ev){
		...
		if( ev.target.className === 'folders' ){
			...
			/*
				1. 阻止默认行为：不仅阻止了选中图片文字的默认行为，还阻止了input的失焦行为，所以在down的时候不能用ev.preventDefault();
				2. 而是用这个可以改变值的 returnValue
			*/
			ev.returnValue = this.rv;

			//this.rv会在重命名&&新建文件夹的时候把值修改为true，以便于修改名称，所以此时，需要把值重置为false，供一次down的时候使用这个false值
			this.rv = false;
	...
	newFolder(){
		...
		//当num对应的数组是为了存放新建文件夹的名称后面的编号的，没有值，说明只有一个名称为新建文件夹的数据
			...
			for(let i=0,len = data[this.id].num.length; i<len+1; i++ ){
				...
				/*
					判断数据的为undefined的有两种情况
						...
						2. 循环数组中每一项都为有效的数字，直到循环至length这一项，因为数组中没有这一项，查找必然得到undefined
				*/
...
/*
	...
	2. 在单击和双击的时候，调用render的时候，
		明白render(e.id)中形参e.id是当前文件夹对应的数据中的id值，
		并不清楚render(id)中形参id是接收的哪里的实参
		
		解析：关键在于闭包
			1. 首先render方法是在类cloudDisk中 （符合闭包条件：函数套函数）
			2. 其次render方法调用是在单击事件和双击事件中 （符合闭包条件：子函数被存起来）
			3. 所以每当双击之后调用render(e.id)，此时e.id为实参传入进去，在render方法这个域中，查找形参id的话，会找到这个实参e.id这个值，
				因此，单击调用render(id)时，此时实参id，在单击事件的事件函数这个域中没有，会继续向上层查找，在render方法这个域中，接收到上次双击事件后传进的e.id的值，由此获取的id值
				（符合闭包条件：子函数使用父函数的参数或变量，此时使用的是参数id，所以每次都会把父函数的这个参数id的值存起来，供下次使用，这就合理解释了render(id)中的id，每次都是使用了上次双击之后的e.id）


	3. 全部点击选中（低级错误）  ->  checkedAll没有选中  --> 在获取这个布尔值的时候，包了中括号，导致获取的返回值错误
		解析：
	...
	4. down的时候，并没有取消选中，而是在move的时候取消选中了，
		解析：
			1. 是因为在move的时候，把数组this.sarr清空，再重新push当前页面数据，因此可以取消选中，
			2. 所以，需要在down的时候，就需要取消选中，把data中的对应的checked的值变为false，然后渲染


	5. 新建文件夹名字相同的时候，弹窗提示了，却新建多个文件夹？
		解析：
			1. 在判断名字相同的语句中，没有写return，导致，继续执行了下面的代码
			2. 发现 return;终止代码执行 这个特点，使用好了，特别省心省力
