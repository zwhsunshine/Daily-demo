...
/*
	...
	2. 在单击和双击的时候，调用render的时候，
		...
		解析：关键在于闭包
			1. 首先render方法是在类cloudDisk中 （闭包条件：函数套函数）
			2. 其次render方法调用是在单击事件和双击事件中 （闭包条件：子函数被存起来）
			3. 所以每当双击之后调用render(e.id)，此时e.id为实参传入进去，在render方法这个域中，查找形参id的话，会找到这个实参e.id这个值，
				...
				（闭包条件：子函数使用父函数的参数或变量，此时使用的是参数id，所以每次都会把父函数的这个参数id的值存起来，供下次使用，这就合理解释了render(id)中的id，每次都是使用了上次双击之后的e.id）
